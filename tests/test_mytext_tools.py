...
import pandas
import makeCSV
from myText_tools import mytext_tools
...
class TestTextObject(unittest.TestCase):
    ...
    def test_getAll(self):
        mydb = TextObject()
        df = mydb.getAll(1, "weather")
        # do we need to format the data, like formatting d3s_data
        #df.to_csv("../dosenet_data/" + "dosenet/" + "weather_data.csv")
        print(df)
        #makeCSV.make_station_files()
