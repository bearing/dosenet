...
if [[ $CHOICE = "all" ]]; then
   ...
   printf "%b\n" "~~ Using: ${BLUE}`mysqldump -u ne170group -p dosimeter_network > ~/backup_all_dosenet.sql`${NC}"
elif [[ $CHOICE = "stations" ]]; then
   printf "%s\n" "~ Just backing up the stations table of the dosimeter_network database"
   printf "%b\n" "~~ Using: ${BLUE}`mysqldump -u ne170group -p dosimeter_network stations > ~/backup_stations_dosenet.sql`"
