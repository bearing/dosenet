...
class Plot(object):
    ...
    def setStationInfo(self, i):
        self.stationRowArrayList.append(
            (i[0], i[1], float(i[2]), float(i[3]), float(i[4]), float(i[5]),
             int(i[6])))
    ...
    def makePlot(self, stationID, unit, error, plotTitle, df, plength):
        try:
            ...
            # setup the plot.ly plot type as a Scatter with points
            #   with panda DataFrames (df)
            trace = Scatter(
                x=df.index,
                y=df[unit],  # changes depending on which units are plotting
                mode='lines+markers',
                error_y=ErrorY(
                    type='data',
                    array=df[error],
            ...
            fontPref = Font(family='Arial, monospace',
                            size=16,
                            color='#000000')
            layout = Layout(title=' '.join((str(stationID), unit, plength)),
    ...
    def makeUnitPlots(
            self, latestTime, latestStationID,
            calibrationCPMtoREM, calibrationCPMtoUSV,
            pointLatLong, plotLengthString, time):
        if latestTime == '':
            print '~~~~ WARNING: Finished list? - No data for this station.'
        else:
            try:
                # Note the negative
                #   that means that we're looking at the past relative to
                #   the latest measurement stored in the DB
        ...
        try:
            ...
            urlA = self.makePlot(
                latestStationID, 'CPM', 'cpmError', self.cpmTitle, df_avg,
                plotLengthString)
            # mREM/hr over numberOfSeconds for all stations
            urlB = self.makePlot(
                latestStationID, 'REM', 'remError', self.remTitle, df_avg,
                plotLengthString)
            # uSv/hr over numberOfSeconds for all stations
            urlC = self.makePlot(
                latestStationID, 'USV', 'usvError', self.usvTitle, df_avg,
                plotLengthString)
    ...
    def plotAll(self):
        ...
        for station in self.getNumberOfStations():
            ...
            # ignore stations
            if ignoreStation == 0:
                print "Ignoring station id={}, name={}".format(
                    stationID, stationName)
                continue
            # builds up a tuple coordinates of the stations,
            #   longitude & latitude
            longlat = [lon, lat]
            point = Point(longlat)
            ...
            for i in dtRows:
                # L --> Latest ...
                (LName, LTime, LDose, Lcpmerror) = i
                LDose = LDose, LDose*Lcpmtorem, LDose*Lcpmtousv
                urlList = []
                urlA = self.makeUnitPlots(
                    LTime, LName, Lcpmtorem, Lcpmtousv, point, plotLength[0],
                    secondsInHour)
                urlB = self.makeUnitPlots(
                    LTime, LName, Lcpmtorem, Lcpmtousv, point, plotLength[1],
                    secondsInDay)
                urlC = self.makeUnitPlots(
                    LTime, LName, Lcpmtorem, Lcpmtousv, point, plotLength[2],
                    secondsInMonth)
                urlD = self.makeUnitPlots(
                    LTime, LName, Lcpmtorem, Lcpmtousv, point, plotLength[3],
                    secondsInYear)
                ...
                try:
                    self.setFeature(
                        point, LName, plotLength, LDose, LTime, urlList)
                except Exception as e:
    ...
    def scpToWebServer(self):
        ...
        #   between GRIM & DECF Kepler works without login
