...
# Globals
# iterations: Default is 1 week (10080 mins / 5 mins)
iterations = 2016
stationID = args.ID[0]
if args.iterations:
    iterations = args.iterations[0]
if args.weeks:
    # number of minutes in a week / 5 minute interval
    iterations = args.weeks[0] * 10080 / 5
cpm = 1.0
cpmError = 1.0
errorFlag = 0
receiveTime = datetime.datetime.now() + datetime.timedelta(days=365)

for t in range(0, int(iterations)):
    # 0,60 --> 1 hr #0,1440 --> 1 day #0,10080 --> 1 week
    ...
    # # Set one station at a time - 2 is first test station
    ...
    try:
        ...
        try:
            ...
            cursor.execute(
                "INSERT INTO " +
                "dosnet(receiveTime, stationID, cpm, cpmError, errorFlag) " +
                "VALUES (%s,%s,%s,%s,%s);",
                (receiveTime, stationID, cpm, cpmError, errorFlag))
